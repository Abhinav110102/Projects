...
//        // Check if location permission is granted before obtaining current location
//        if (foregroundPermissionApproved()) {
//            // Use the last known location as an example
//            val location = foregroundOnlyLocationService
//        } else {
...
//    }
    ...
    if (foregroundPermissionApproved()) {
        // Use the last known location as an example
        val location = foregroundOnlyLocationService?.getLastLocation()
        if (location != null) {
            updateMap(location.latitude, location.longitude)
        } else {
            // If last known location is not available, you can set a default location
            val defaultLatitude = 34.00071 // Example latitude
            val defaultLongitude = -81.0348133 // Example longitude
    ...
    // Update the map with the given location
...
//        val longitude = location.longitude
    ...
    private fun logResultsToScreen(output: String, location: Location) {
        val outputWithPreviousLogs = "$output\n${outputTextView.text}"
        outputTextView.text = outputWithPreviousLogs
        updateMap(location.latitude, location.longitude)
        //location.let{updateMap(it.latitude, it.longitude)}
    }
    /**
    ...
    private inner class ForegroundOnlyBroadcastReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (location != null) {
                logResultsToScreen("Foreground location: ${location.toText()}", location)
            }
        }
    }
}
